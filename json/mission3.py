...
def main():
    ...
    for href_page in href_pages:
        ...
        for href_card in href_cards:
            ...
            result_json = []
            for list_item, price_item, name, count, price, old_price, li_desc in zip(description, price, name, count, price, old_price, description_li):
                result_json.append({
                    'categories': 'watch',
                    'name': name,
                    'article': article,
                    'description ': {
                        [li['id'] for li in description_li]: [x.split(':')[-1] for x in list_item][0],
                        ...
                        [x.split(':')[0] for x in list_item][4]: [x.split(':')[-1] for x in list_item][4],
                        [x.split(':')[0] for x in list_item][5]: [x.split(':')[-1] for x in list_item][5],
                        [x.split(':')[0] for x in list_item][6]: [x.split(':')[-1] for x in list_item][6],
                        [x.split(':')[0] for x in list_item][7]: [x.split(':')[-1] for x in list_item][7],
                    },
                    'count': count,
                    'price': price_item,
                    'old_price': old_price
